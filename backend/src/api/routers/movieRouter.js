...
router.get('/get', async (req, res) => {
	try {
		...
		res
			.status(500)
			.send({ status: 'Error with getting movies', error: err.message });
	}
});
...
router.get('/get/:id', async (req, res) => {
	try {
		...
		res
			.status(500)
			.send({ status: 'Error with getting movie', error: err.message });
	}
});
...
router.delete('/delete/:id', auth, async (req, res) => {
	try {
		...
		res
			.status(500)
			.send({ status: 'error with deleting movie', error: err.message });
	}
});
